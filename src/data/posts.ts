...
export type PostImage = {
  src: string;
  alt: string;
  /** Crédito da imagem (ex.: Unsplash). Vazio quando enviada pelo escritório. */
  credit?: string;
  creditUrl?: string;
};
...
export type Post = {
  ...
  updatedAt?: string; // ISO — última edição (dateModified no SEO)
  excerpt: string;
...
## Quando a agência ou a companhia cancela
...
Guarde contratos, comprovantes de pagamento e todas as conversas com a empresa: essa documentação é essencial para buscar seus direitos.`,
  },
  {
    ...
    body: `Grande parte dos litígios nasce de contratos mal redigidos ou assinados sem análise. Cláusulas ambíguas, obrigações desequilibradas e omissões importantes só aparecem quando o problema já existe.

## O que a revisão contratual verifica
...
Ajustar um contrato antes da assinatura é significativamente mais simples do que discuti-lo em juízo. Para empresas, a revisão periódica dos modelos utilizados no dia a dia é uma medida de gestão de risco básica.`,
  },
  {
    ...
    body: `A desapropriação é o procedimento pelo qual o poder público adquire compulsoriamente um bem particular, por necessidade ou utilidade pública, ou por interesse social. A Constituição exige, como regra, indenização justa e prévia.

## O que o proprietário pode discutir
...
O proprietário pode contestar o valor ofertado, apresentar avaliação própria e discutir juros, correção e honorários. O acompanhamento técnico desde a fase administrativa faz diferença no resultado.`,
  },
  {
    ...
    body: `As férias-prêmio são um direito dos servidores públicos estaduais de Minas Gerais previsto na legislação estadual: períodos de descanso remunerado adquiridos a cada ciclo de efetivo exercício.
...
Revisar o contrato antes de assinar — e guardar toda a documentação da locação — evita surpresas na renovação ou na devolução do imóvel.`,
  },
  {
    ...
    body: `Erros de enquadramento, cobranças em duplicidade e normas declaradas inconstitucionais fazem com que contribuintes paguem mais tributo do que devem. Nesses casos, a legislação prevê a restituição ou a compensação dos valores.
...
/** Slug seguro a partir de um título (sem acentos, kebab-case). */
