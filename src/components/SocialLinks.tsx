...
/**
 * Ícones de marca em SVG inline (o lucide-react removeu os ícones de
 * marca por serem trademark). Traçado único em `currentColor`, viewBox
 * 24×24 — herdam a cor e o hover do link.
 */
...
export default function SocialLinks({ className = "" }: SocialLinksProps) {
  const links = order
    .map((key) => ({ key, href: site.social[key], ...icons[key] }))
    .filter((s) => Boolean(s.href));

  return (
    <ul className={`flex flex-wrap gap-3 ${className}`}>
      {links.map(({ key, href, title, path }) => (
        <li key={key}>
          <a
            href={href}
            target="_blank"
